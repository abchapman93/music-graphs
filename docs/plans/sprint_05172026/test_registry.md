...
| M7 | `/graph/bowie-covers` works identically; at least one Bowie original + cover pair both have working Spotify embeds | ✅ Note: The original David Bowie songs aren't linked to David Bowie tracks. Those could be either separate nodes or the same node. |
