...
| Q1 | Q | Edit → Save round-trips: PUT /api/cards/<g>/<s> writes file and refreshes panel | pytest: tmp graph + card; PUT new body; file on disk has new body, frontmatter unchanged; lint exit 0 | ✅ `tests/test_routes.py::test_put_card_round_trips` — frontmatter bytes split via `lib.cards.split_frontmatter_bytes`, body replaced, payload returned. Frontmatter block compared byte-for-byte. |
| Q2 | Q | Lint failure on save reverts the file and returns 422 with error message | pytest: PUT body containing a dangling wikilink; file restored bit-for-bit; response 422 + error string | ✅ `tests/test_routes.py::test_put_card_lint_failure_reverts` — dangling `[[person:nope-not-real]]` triggers `lint_graph()` error; file restored from in-memory backup; 422 with `stdout` containing the offending slug. |
| Q3 | Q | Cancel discards changes without writing | DOM smoke: edit → cancel → file mtime unchanged | ✅ `tests/test_routes.py::test_cancel_does_not_touch_file` — opening editor via GET `/api/card/<g>/<s>` returns `body_md` without touching disk; mtime_ns identical before/after. Cancel button in `graph.js::exitEditMode` re-renders from cached payload, never calls fetch. |
| R1 | R | POST /api/notes/<g> creates a new note card with frontmatter + body | pytest: post {type:"note", title, body, source_slug, source_type}; file exists at `note-<slug>.md`; lint exit 0 | ✅ `tests/test_routes.py::test_post_note_creates_card` — route delegates to `write_card` skill lib; `note-studio-anecdote.md` written with `type: note`, quoted `name`, and body; in-process `lint_graph(gdir)` returns no errors; response includes `{slug, type, name, url}`. |
| R2 | R | New note card has a wikilink edge back to the source node | pytest: source card body contains `[[note:<new-slug>]]`; reciprocal edge written if symmetric | ✅ `tests/test_routes.py::test_post_note_writes_edge_back` — route delegates to `add_edge` skill lib (one-way, no relationship verb — skill default); source `person-alpha.md` body gains `[[memory:<slug>...]]` wikilink. |
| R3 | R | Lint failure on note create reverts both files and returns 422 | pytest: force lint fail (e.g., invalid frontmatter inject); neither new file nor source mutation persists | ✅ `tests/test_routes.py::test_post_note_lint_failure_reverts` — `lint_graph` monkeypatched to return a synthetic error; new card file deleted and source restored from in-memory `source_backup` bytes (compared byte-for-byte); 422 with `error="lint failed; changes reverted"` and `stdout` containing the error string. |
